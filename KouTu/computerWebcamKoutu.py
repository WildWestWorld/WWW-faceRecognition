...
def process_frame(img,imgArr):
    ...
    # #mask若为true 该位置就是img
    # #若为false 该位置就是img_without_bgc
    bgc_img_KT_addPerson = np.where(mask_3, img, bgc_img_KT)

    # 把加上人像后的抠图放回到原图的位置
    bgc_img[Top:Bottom, Left:Right, 0:] = bgc_img_KT_addPerson;
...
while cap.isOpened():
    ...
    ## !!!处理帧函数
    frame = process_frame(frame,imgArr)
